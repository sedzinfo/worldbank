...
worldbank<-function() {
  ...
  format_bignum=function(n) {
    case_when(n>=1e12~paste(round(n/1e12),"Tn"),
  ...
  shinyApp(ui=tagList(tags$head(tags$script('var dimension = [0, 0];
                           ...
                           .navbar {min-height:25px;}'))
                                 ...
                                 tabPanel("Country Comparison",
                                          ...
                                          selectizeInput(inputId="indicator_country_comp",
                                                         ...
                                                         width="100%"),
                                          plotlyOutput("plot_country_comp")),
                                 tabPanel("Indicator Comparison",
                                          ...
                                          selectizeInput(inputId="country_indicator_comp",
                                                         ...
                                                         selected=c("Greece"),
                                                         width="100%"),
                                          plotlyOutput("plot_indicator_comp")),
                                 tabPanel("Pyramid",
           ...
           server=function(input,output) {
             output$plot_country_comp<-renderPlotly({
             ...
             output$plot_indicator_comp<-renderPlotly({
             ...
             output$plot_pyramid<-renderPlotly({
               ...
               plot_ly(temp,
                 ...
                 layout(bargap=.1,barmode='overlay',
                        ...
                        font=font_style)%>%
                 animation_opts(frame=500,transition=1,easing="linear",redraw=FALSE,mode="immediate")
             })
             output$plot_bar<-renderPlotly({
               ...
               temp$year<-droplevels(temp$year)
               temp<-mutate(group_by(temp,year),rank=order(order(value,year,decreasing=TRUE)))
               if(nrow(temp)>1) {
                 plot_ly(temp,
                         x=~value,
                         y=~country,
                   ...
                   layout(xaxis=list(title=""),
                          ...
                          font=font_style,
                          showlegend=FALSE) %>%
                   add_text(text=format_bignum(temp$rank),textposition="right")%>%
                   animation_opts(frame=500,transition=1,easing="linear",redraw=FALSE,mode="immediate")
               }
             })
             ...
             output$index_table=DT::renderDataTable({
               ...
               formatStyle(result,names(result),0,target='row',lineHeight='80%')
             })
           }
  )
}
